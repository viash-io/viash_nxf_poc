...
// A process that filters out output_one from the output Map
...
// A process that filters out output_multi from the output Map
...
// A process that filters out output_opt from the output Map
...
workflow {
  ...
  result \
    | filterOutput_one \
    | view{ "Output for output_one: " + it[1] }


  result \
    | filterOutput_multi \
    | view{ "Output for output_multi: " + it[1] }


  result \
    | filterOutput_opt \
    | view{ "Output for output_opt: " + it[1] }
...
workflow test {
  ...
  main:
  params.test = true
  params.poc.output = "poc.log"

  Channel.from(rootDir) \
    | filter { params.poc.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.poc.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | poc
