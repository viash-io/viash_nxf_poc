...
process normalize_process {
  ...
  stageInMode "symlink"
  container "${container}"
  publishDir "mytest", mode: 'copy', overwrite: true
  //publishDir "${params.publishDir}/${id}/", mode: 'copy', overwrite: true
  input:
...
workflow normalize {
  ...
  result_ = normalize_process(id_input_output_function_cli_params_) \
    ...
    | map{ id, output, input ->
        def parsedOutput = fun.arguments
        ...
        new Tuple2(id, parsedOutput) // TODO: output additional args as well, if any
      }
...
workflow {
  ...
  for (entry in fun[fname].arguments) {
    def name = entry.value.name
    if (params[name] != null) {
      fun[fname].arguments[name].value = params[name]
    }
  }

  def inputFiles = fun.normalize
    .arguments
    .findAll{ key, par -> par.type == "file" && par.direction == "Input" }
    .collectEntries{ key, par -> [(par.name): file(fun[fname].arguments[par.name].value) ] }

  def ch_ = Channel.from("").map{ s -> new Tuple2(id, inputFiles)}
