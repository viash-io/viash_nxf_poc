...
include { poc2 } from "./target/nextflow/poc2/main.nf" params(params)
include { overrideOptionValue }         from  '../../utils.nf'
...
workflow run_main {
    ...
    output_ = input_one
      ...
      | map{ [ "input", [ input_one: it[0], input_multi: it[1] ], params ] }

      // problem 2: overriding parameters is clunky
      | map { overrideOptionValue(it, "poc", "string", it[1][0].name) }
      | poc
